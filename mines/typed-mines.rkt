...
(define tile:plain%
  (class object%
    (super-new) ; added for type checking
    (: state State)
     ...
     (augment*
      [on-close ; stop the timer, in case it's running
       (lambda ()
         ...
         (inner (void) #;'() on-close))])
     (super-new)) ; removed super0instantiate
    ()
   [label "Minesweeper"] 
...
(define (make-centering-pane parent)
  (let ([p (new vertical-pane% [parent parent])#;(make-object vertical-pane% parent)])
    (send p set-alignment 'center 'center)
    p))

(define time-display
  (new message% [label "Time: 00000"] [parent (make-centering-pane panel)])) ;removed make-object
(new button%
     [label "Reset"]
     [parent (make-centering-pane panel)]
     [callback (lambda (b e) (send board-canvas reset))])
(define count-display
  (new message% [label "Count: 000"] [parent (make-centering-pane panel)]))
...
(define board-canvas% 
  (class canvas%
    ...
    (: clicking (Option (Instance Tile:Plain%)))
    ...
    (: clicking-right? Boolean)
    (define clicking-right? #f) ; #t => right-click in progress
    (: area-hilite (Option (Instance Tile:Plain%)))
    (define area-hilite #f)     ; tile with mouse pointer over it
    (: area-hilites (Listof (List (Instance Tile:Plain%) Integer Integer))) ; to appease the type-checker slightly until I figure out what type this really should be
    (define area-hilites null)  ; tiles+locs hilited due to mouse-over
    (: ready? Boolean)
    (define ready? #t)          ; #t => accept clicks
    (: start-time (Option Integer))
              ...
              (make-object
               (class timer% '()
                 ...
                 (override*
                  [notify
                   (lambda ()
                     (let: ([e : Integer (- (current-seconds) (assert start-time))])
                       (when (> e elapsed-time)
                         (set! elapsed-time e)
                         (set-time e))))])
                 #;(super-instantiate ()))))
    ...
    (send dc set-text-background (assert BG-COLOR))
    (send dc set-brush (send the-brush-list find-or-create-brush 
                             (assert BG-COLOR) 'solid))))

;; Make the board canvas:
(: board-canvas (Instance Board-Canvas%))
(define board-canvas 
  (new board-canvas% [frame frame])
  #;(make-object board-canvas% frame))
