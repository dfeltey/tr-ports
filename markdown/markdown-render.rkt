...
(require/typed racket 
               [regexp-replaces ((U String Bytes) (Listof (List Regexp String)) -> String)]
               [string-trim (String -> String)]
               [regexp-match? (Regexp String -> Boolean)]
               [append* (All (A) ((Listof A) (Listof (Listof A)) -> (Listof A)))])
(require  "base-render.rkt"
         typed/scribble/core
         (except-in "private/render-utils.rkt" part-style?)
         (except-in scribble/text/wrap wrap-line))
...
(define-type Render-Class%
   (Class 
          [traverse ((Listof part) (Listof Path-String) -> (HashTable Symbol Procedure))]
        ...
        (and (zero? (table-ticks-depth))
             ...
             (let ([s (element-style i)])
               (or (eq? 'tt s)
                   (and (style? s)
                        (style-name s)
                        ; (syle-name s) needs to be a string below
