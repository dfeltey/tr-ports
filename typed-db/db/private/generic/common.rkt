...
(require/typed racket 
               [memq (All (a) (Any (Listof a) -> (Option (Listof a))))]
               [hash-remove (All (a b) ((HashTable a b) Any -> (HashTable a b)))])
...
(define-type Statement-Cache%
  (Class #:implements Transactions%
         ...
         [prepare1* (Symbol Symbol Boolean (Option Symbol) -> (Instance Prepared-Statement<%>))]
         [classify-stmt (Symbol -> (Option Symbol))]))
...
(define statement-cache%
  (class transactions%
    ...
    (define/public (get-cached-statement stmt)
      (let ([cached-pst (hash-ref pst-cache stmt (thunk #f))]) ;add thunk to false arg..
        (cond [cached-pst
