...
(define-type Pasteboard%
  (Class #:implements mred:Pasteboard%
         ...
         [set-double-click-action ((Any -> Void) -> Void)]
         [set-single-click-action ((Any -> Void) -> Void)]
  ...
  (define pasteboard%
    (class mred:pasteboard%
      ...
      (define button-map '((left #f #f #t)
                           (middle #t #f #t)
                           (right #f #t #f)))
      (: do-on-double-click (U 'flip (Any -> Void)))
      (define do-on-double-click 'flip)
      (: do-on-single-click (Any -> Void))
      ...
      (define last-click #f)
      (: regions (Listof Region)) ; not sure if this is write, but a decent starting guess
      (define regions null)
      (: update-region (Region -> Void))
      (: get-snip-bounds ((Instance Card%) -> (Values Real Real Real Real)))
      (: get-reverse-selected-list (-> (Listof (Instance Card%))))
      (: make-overlapping-list ((Instance Card%) (Listof (Instance Card%)) Boolean -> (Listof (Instance Card%))))
      (: for-each-selected (((Instance mred:Snip%) -> Any) -> Void))
      (private*
	[get-snip-bounds
	 (lambda: ([s : (Instance Card%)])
	...
	[for-each-selected
	 (lambda: ([f : ((Instance mred:Snip%) -> Any)])
        ...
        [make-overlapping-list
         (lambda: ([s : (Instance Card%)] [so-far : (Listof (Instance Card%))] [behind? : Boolean])
           (let-values ([(sl st sr sb) (get-snip-bounds s)])
             (let: loop : (Listof (Instance Card%)) ([t (find-first-snip)][so-far so-far][get? (not behind?)])
               (cond
                 [(not t) so-far]
                 [(eq? s t) (if behind?
		...
		[get?
		 (let ([l (if (and (not (memq t so-far))
				   (let-values ([(tl tt tr tb) 
						 (get-snip-bounds (cast t (Instance Card%)))])
				     (and (or (<= sl tl sr)
					      (<= sl tr sr))
					  (or (<= st tt sb)
					      (<= st tb sb)))))
			      (make-overlapping-list (cast t (Instance Card%)) (cons (cast t (Instance Card%)) so-far) behind?)
			      so-far)])
        ...
        [get-reverse-selected-list
         (lambda ()
           (let: loop : (Listof (Instance Card%)) ([s (find-next-selected-snip #f)][l : (Listof (Instance Card%)) null])
             (if s
                 (loop (find-next-selected-snip s) (cons (cast s (Instance Card%)) l))
                 l)))]
        [shuffle
         (lambda: ([selected-list : (Listof (Instance Card%))]) ; cards to shuffle in back->front order
           (let*: ([permuted-list : (Listof (Instance Card%))
                  ...
                  [sel-loc-list : (Listof (Pairof Real Real)) (map get-pos selected-list)]
                  [perm-loc-list ((inst map (Pairof Real Real) (Instance Card%)) get-pos permuted-list)])
             (for-each
              (lambda: ([s : (Instance mred:Snip%)] [start-pos : (Pairof Real Real)] [end-pos : (Pairof Real Real)])
                (let* ([sx (car start-pos)]
                       [sy (cdr start-pos)]
                       [ex (car end-pos)]
                       [ey (cdr end-pos)]
                       [steps (max 1 (floor (/ 50 (length selected-list))))])
                  (let: loop : Void ([i : Real 1])
                    (unless (> i steps)
	...
	[after-select
	 (lambda: ([s : (Instance mred:Snip%)] [on? : Any])
	   ...
	   (unless (or (not on?) selecting?)
		 ...
		 (begin
		   (begin-edit-sequence)
		   (let ([l (make-overlapping-list (cast s (Instance Card%)) (list (cast s (Instance Card%))) select-backward?)])
		     (for-each (lambda: ([i : (Instance mred:Snip%)]) (add-selected i)) l))
		   (when raise-to-front?
		     (let: loop : Void ([snip (find-next-selected-snip #f)][prev : (Option (Instance mred:Snip%)) #f])
		       (when snip
			 (if prev
			     (set-after snip prev)
			     (set-before snip #f))
	...
	[on-interactive-move
	 (lambda (e)
	   ...
	   (for-each-selected (lambda: ([snip : (Instance mred:Snip%)]) (send (cast snip (Instance Card%)) remember-location this)))
	   (set! dragging? #t))])
      (override*
	[interactive-adjust-move
	 (lambda: ([snip : (Instance mred:Snip%)] [xb : (Boxof Real)] [yb : (Boxof Real)])
	   (super interactive-adjust-move snip xb yb)
	   (let-values: ([([l : Real] [t : Real] [r : Real] [b : Real]) (get-snip-bounds (cast snip (Instance Card%)))])
	     (let-values: ([([rl : Real] [rt : Real] [rw : Real] [rh : Real])
			   (let: ([r : (Option Region) (send (cast snip (Instance Card%)) stay-in-region)])
			     (if r
	...
	[after-interactive-move
	 (lambda: ([e : (Instance mred:Mouse-Event%)])
           (when dragging?
             ...
             (for-each-selected (lambda: ([snip : (Instance mred:Snip%)]) (send (cast snip (Instance Card%)) back-to-original-location this)))
             (let: ([cards : (Listof (Instance Card%)) (get-reverse-selected-list)])
	 ...
	 (lambda: ([e : (Instance mred:Mouse-Event%)])
	   (let: ([click : (Option (U 'left 'middle 'right)) (let: ([c : (Option (U 'left 'middle 'right)) (or (and (send e button-down? 'left) 'left)
		 ...
		 (unless (or (not click-base) (not s) (eq? s click-base))
		   (no-selected))
		 (set! click-base (cast s (Instance Card%)))) ; there doesn't seem to be a better way to get this to typecheck than to cast s to a Card% instance
	...
	[get-all-list
	 (lambda ()
	   (let: loop : (Listof (Instance mred:Snip%)) ([t : (Option (Instance mred:Snip%)) (find-first-snip)][accum : (Listof (Instance mred:Snip%)) null])
	     (cond
	      [(not t) (reverse accum)]
  ...
  (define-type Table%
    (Class #:implements mred:Frame%
         ...
         [add-card ((Instance Card%) Real Real -> Void)]
  ...
  (define table%
	...
	[add-cards
	 (opt-lambda: ([cards : (Listof (Instance Card%))] [x : Real] [y : Real] [offset : (Integer -> (Values Real Real)) (lambda (p) (values 0 0))])
	...
	[create-status-pane
	 (lambda ()
	   (let ([p (new mred:horizontal-pane% [parent this])#;(make-object mred:horizontal-pane% this)])
		...
		(callback
		 (let ([show-help (show-scribbling mod tag)])
		   (lambda x
		     (show-help))))))])
      (begin
        (: msg (Option (Instance mred:Message%)))
        (define msg #f)
	...
	(define in-sequence 0))
      (: position-cards : (Listof (Instance Card%)) Real Real (Integer -> (Values Real Real)) ((Instance Card%) Real Real -> Any) -> Void)
	...
	[position-cards-in-region
	 (lambda (cards r set)
      ...
      (begin
        ...
        (define c (new mred:editor-canvas% [parent this] [editor #f] [style '(no-vscroll no-hscroll)]))
